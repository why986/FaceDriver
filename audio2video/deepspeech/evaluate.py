...
def create_inference_graph(batch_size=1, n_steps=16, tflite=False):
    ...
    input_samples = tfv1.placeholder(tf.float32, [int(Config.audio_window_samples)], 'input_samples')
    ...
    if batch_size <= 0:
        ...
        previous_state = tfv1.nn.rnn_cell.LSTMStateTuple(previous_state_c, previous_state_h)
...
def get_prob(audio_path):
    ...
    features = create_overlapping_windows(features).eval(session=DeepSpeechGlobalSession.session)
    features_len = features_len.eval(session=DeepSpeechGlobalSession.session)

    probs = DeepSpeechGlobalSession.outputs['outputs'].eval(feed_dict={
        DeepSpeechGlobalSession.inputs['input']: features,
        DeepSpeechGlobalSession.inputs['input_lengths']: features_len,
        DeepSpeechGlobalSession.inputs['previous_state_c']: previous_state_c,
        DeepSpeechGlobalSession.inputs['previous_state_h']: previous_state_h,
    }, session=DeepSpeechGlobalSession.session)
...
def run_script():
    create_flags()
    absl.app._run_init(None, absl.app.parse_flags_with_usage)
    create_session()
